...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const postPrompt =
        "return the data as an object of { 'key': 'value'}, if you dont know an answer for any individual item, please keep the structure of { 'key':'value' } but return the value be ''. If you do know the answer replace the value with the correct data, Keep context, dont return anything you are unsure of. return only the specified JSON object of { 'key': 'value' }. Respond ONLY with a Valid JSON message, do not respond with text such as 'I Dont Know' or 'Im sorry' , Respond ONLY with a JSON object with all of the keys present and NO additional text. If you do not have the answer for a paticular query return '' for the value. If you do know the answer put this in the place of value. If the provided text does not contain the information required to answer a query, then simply return '' as the value.";
      let tagResults = {};
